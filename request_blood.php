...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($age) || empty($bloodType) || empty($quantity) || empty($unit) || empty($hospital) || empty($dateNeeded) || empty($contact) || empty($reason)) {
        ...
        $insertQuery = "INSERT INTO blood_requests (request_by, name, age, blood_group, quantity, hospital_unit, hospital_name, date_needed, contact, reason) VALUES ('$loggedInUsername', '$name', '$age', '$bloodType', '$quantity', '$unit', '$hospital', '$dateNeeded', '$contact', '$reason')";
